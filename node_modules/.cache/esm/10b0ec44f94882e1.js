let Headers,fetch,fs;_f94‍.w("node-fetch",[["Headers",["Headers"],function(v){Headers=v}],["default",["fetch"],function(v){fetch=v}]]);_f94‍.w("fs",[["default",["fs"],function(v){fs=v}]]);



// Get the query ID from the command line arguments
const queryID = process.argv[2]; // Example Query ID: 1258228
_f94‍.g.console.log("retrieving data for query " + queryID + "..."); // Check that Query ID is passing through
...
async function fetchQueryData(queryID) {
  try {
    ...
    fs.writeFile(fileName, resultsData, (err) => {
      if (err) {
        _f94‍.g.console.error(`Error writing the CSV file: ${fileName}`, err);
      } else {
    ...
    _f94‍.g.console.error("Error:", error.message);
  }
}
