...
with cex_kr as (
    select * from "cex.addresses" where cex_name in ('Binance', 'Bitget', 'Bybit')
), amount_series as (
SELECT date_trunc('week', block_time) as dt, cex_name, sum(amount) as amount
FROM (
    -- outbound transfers
	SELECT cex.cex_name, block_time, -1*cast(value as int8)/1e18 AS amount
    FROM "ethereum.traces" tr
    LEFT JOIN cex_kr AS cex ON cex.address = tr."from"
    WHERE tr."from" = cex.address
    AND success
    ...
    SELECT 
        cex.cex_name, 
        block_time,
        cast(-1 as int8)*cast(gas_price as int8)*cast(gas_used as int8)/1e18 AS amount
    FROM "ethereum.transactions" tr
    JOIN cex_kr AS cex ON tr."from" = cex.address
    WHERE tr."from" = cex.address
) t group by dt, cex_name),
date_period as (
    select *
    from (select distinct dt from amount_series) ddt
    cross join (select distinct cex_name from cex_kr) dcn
)
select 
    dp.dt,
    dp.cex_name,
...
from date_period dp
left join amount_series a on dp.dt = a.dt and dp.cex_name = a.cex_name
--group by dp.dt, dp.cex_name
order by dp.cex_name
